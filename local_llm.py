...
class Jarvis:
    ...
    def __init__(self, config: Optional[JarvisConfig] = None):
        ...
        self.tts = Kokoro(
            ...
            voice="af_nova", #default female american voice
        ...
        self.speaking_speed = 1 #how fast TTS goes
    ...
    def _init_llm(self):
        ...
        self.llm = QwenChat(auto_append_conversation=self.config.auto_append_messages,name=self.config.wake_word)


        """def display_image_for_user(prompt):
            print(f"MODEL IMAGE PROMPT: {prompt}")
            return {"result":"Image was generated and is being displayed to user"}"""
      

        """self.llm._set_emotion_based_system_prompt(emotion="aroused")
    ...
    def _execute_prompt_internal(self, prompt: str):
        ...
        try:
            ...
            #emotion_prompt = self.llm._system_prompt_emotional_states(emotion="aroused")
            memory_system_prompt = f"""
                You are conversing with {self.primary_speaker or 'the user'}. Here is the most recent part of your conversation and related memories:

                <your_memory>
                {memory_context}
                </your_memory>

                Use your memory context to provide more relevant and personalized responses."""
            ...
            with self.state_lock:
                if self.state == JarvisState.THINKING:
                    ...
                    self.tts.generate_speech_async(response, speed=self.speaking_speed)        
