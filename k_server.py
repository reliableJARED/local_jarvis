...
def generate_image_thread(prompt, job_id):
    ...
    try:
        ...
        #generator = ImageGenerator()
        output_path = f"./{IMAGE_FOLDER}/images/img_{str(uuid.uuid4())}.png"
        ...
        generation_status[job_id] = {
            ...
            'progress': 100,
            'image_path': output_path.replace(f'./{IMAGE_FOLDER}/', ''),
...
def generate_image():
    ...
    try:
        ...
        # Check maximum length to prevent overly long prompts
        if len(prompt) > 320:
            return jsonify({
                'success': False,
                'error': 'Prompt must be less than 320 characters'
            }), 400
        ...
        return jsonify({
            ...
            'prompt': prompt  # Return the prompt for debugging/confirmation
        })
...
IMAGE_PROMPT_TEMPLATE = '''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <p style="color: #666;">Create stunning images using natural language descriptions</p>
        </div>
        ...
        <div class="generate-section">
            ...
            <div class="form-container">
                <div class="form-group">
                    <label class="form-label" for="prompt">Image Description</label>
                    <textarea 
                        class="prompt-textarea" 
                        id="prompt" 
                        placeholder="Describe the image you want to generate in detail. For example:
...
Your prompt will be automatically saved as you type."
                        maxlength="320"
                    ></textarea>
                    <div class="character-count" id="charCount">0 / 320 characters</div>
                    <div class="saved-indicator" id="savedIndicator">✓ Prompt automatically saved</div>
                </div>
        ...
        <div class="tips-section">
            ...
            <ul class="tips-list">
                <li><strong>Be descriptive:</strong> Include style (photograph, digital art, painting), subject details, appearance, pose, clothing, and lighting</li>
                <li><strong>Use specific terms:</strong> Instead of "nice hair" try "flowing auburn hair with soft waves"</li>
                <li><strong>Include technical details:</strong> Add terms like "8k", "high resolution", "professional photography" for quality</li>
                <li><strong>Mention lighting:</strong> "soft natural light", "golden hour", "studio lighting" can dramatically improve results</li>
                <li><strong>Auto-save:</strong> Your prompt is automatically saved as you type and will be restored when you return</li>
                <li><strong>Start with style:</strong> Begin with "photograph of", "digital art of", "oil painting of" etc.</li>
            </ul>
        </div>
    </div>
    ...
    <script>
        ...
        function updateCharCount() {
            ...
            charCount.textContent = `${count} / 320 characters`;
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.status === 'completed') {
                            ...
                            statusText.innerHTML = `✅ ${data.message}<br><a href="/folder/images" style="color: white; text-decoration: underline; font-weight: bold;">→ View Generated Image</a>`;
        ...
        function clearForm() {
            // Show confirmation dialog since this will clear saved data
            if (confirm('This will clear the text and remove your saved prompt. Are you sure?')) {
                document.getElementById('prompt').value = '';
                setCookie('imagePrompt', '', -1); // Delete the cookie
                updateCharCount();
        ...
        function savePromptToCookie() {
            const prompt = document.getElementById('prompt').value;
            setCookie('imagePrompt', prompt, 30); // Save for 30 days
        ...
        function loadPromptFromCookie() {
            const savedPrompt = getCookie('imagePrompt');
            if (savedPrompt) {
        ...
        document.getElementById('prompt').addEventListener('input', function() {
            updateCharCount();
            // Debounce saving to avoid too many cookie writes
            clearTimeout(window.saveTimeout);
            window.saveTimeout = setTimeout(savePromptToCookie, 500);
        });
        ...
        // Focus on textarea when page loads and update char count
        document.addEventListener('DOMContentLoaded', function() {
            loadPromptFromCookie(); // Load saved prompt first
            document.getElementById('prompt').focus();
            updateCharCount();
        });
    </script>
